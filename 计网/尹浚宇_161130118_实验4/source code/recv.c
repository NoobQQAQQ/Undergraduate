...
// 功能: 判断ping的是否是本机
// 输出: 是返回1, 否则返回0
int dst_is_host(char *dst_ip)
{
    for (int i = 0; i < device_item_nr; i++)
        if (!strcmp(dst_ip, device_table[i].ip_addr))
            return 1;
    return 0;
}
...
// 功能: 查询设备表找到源ip对应的源端口
// 输出: 源端口
char* get_if(char *src_ip)
{
    for (int i = 0; i < device_item_nr; i++)
        if(!strcmp(src_ip, device_table[i].ip_addr))
            return device_table[i].interface;
}
...
// 功能: 发送icmp回应
// 输出: 无
void send_reply(char *buffer, int frame_len)
{
    ...
    // 互换源和目的mac
    memcpy(src_mac, buffer, 6);
	memcpy(dst_mac, buffer + 6, 6);
    memcpy(buffer, dst_mac, 6);
	memcpy(buffer + 6, src_mac, 6);
    // 互换源和目的ip, 重算检验和
    struct ip *ip = (struct ip*)(buffer + 14);
    uint32_t src_ip = ip->ip_dst.s_addr;
    uint32_t dst_ip = ip->ip_src.s_addr;
    ip->ip_src.s_addr = src_ip;
    ip->ip_dst.s_addr = dst_ip;
    ip->ip_sum = 0;
    ip->ip_sum = checksum((char*)ip, sizeof(struct ip)); // 检验和
    // 修改icmp类型
    struct icmp *icmp = (struct icmp*)(buffer + 34);
    icmp->icmp_type = ICMP_ECHOREPLY;
    icmp->icmp_cksum = 0;
    icmp->icmp_cksum = checksum((char*)icmp, frame_len - 34); 

    char *interface = get_if(inet_ntoa(ip->ip_src)); // 用源ip查设备表获取源端口

	struct sockaddr_ll device; // 配合sendto()函数来发送以太网帧
    fill_sockaddr_ll(&device, src_mac, interface); // 填充sockaddr_ll结构
  
    sendto(sock_fd, buffer, frame_len, 0, (struct sockaddr*) &device, sizeof(device));

    close(sock_fd); // 关闭套接字描述符
}

// 函数名: change_frame
// 输入: 指向待修改的以太网帧的指针, 存放源MAC地址的数组, 存放目的MAC地址的数组, 目的ip, 源ip
// 功能: 修改以太网帧
// 输出: 无
void change_frame(uint8_t *ether_frame, uint8_t src_mac[6], uint8_t dst_mac[6])
{
    // 修改以太网帧头部
    ...
    // 修改ip包头部
    struct ip *ip = (struct ip*)(ether_frame + 14);
    ip->ip_ttl -= 1; // ttl - 1
    // 重算检验和
    ip->ip_sum = 0;
    ip->ip_sum = checksum((char*)ip, sizeof(struct ip)); // 检验和
...
// 功能: 转发收到的包
// 输出: 无
void forward(char *buffer, int frame_len, char *dst_ip)
{
    int sock_fd; // 套接字描述符
    // 创建套接字并处理创建套接字失败的情况
    if ((sock_fd = socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL))) < 0)
    ...
    if (interface == NULL)
    {
        printf("no need for transfer\n");
        return;
    }
    get_src_mac(src_mac, interface); // 根据源端口号, 获取源mac地址

    fill_sockaddr_ll(&device, src_mac, interface); // 填充sockaddr_ll结构
    change_frame(buffer, src_mac, dst_mac); // 改以太网帧
...
void IP_analyse(char* buffer, int frame_len)
{
    ...
    if (dst_is_host(dst_ip)) // 目的ip就是本机
    {
        printf("dst is myself\n");
        struct icmp *icmp = (struct icmp*)(buffer + 34);
        if (icmp->icmp_type == ICMP_ECHO)
        {
            printf("receive a icmp for request\n");
            send_reply(buffer, frame_len); // 发送icmp echo回应
            return;
        }
        else if(icmp->icmp_type == ICMP_ECHOREPLY)
        {
            printf("receive a icmp for reply\n");
            return;
        }     
    }

    else if(is_router()) // 本机是路由器
    {
        printf("dst is not myself, need forwarding\n");
        forward(buffer, frame_len, dst_ip); // 转发收到的包
        return;
    }

    // 其他情况不处理
    printf("host is not a router, drop the packet\n");
}
