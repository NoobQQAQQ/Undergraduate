...
void SDL_FillRect(SDL_Surface *dst, SDL_Rect *dstrect, uint32_t color) {
    ...
    int dx = 0, dy = 0, w = dst->w, h = dst->h; // set initial value as desrect == NULL

    if (dstrect) // if dstrect != NULL, modify needed values
	{
        dx = dstrect->x;
        dy = dstrect->y;
        w = dstrect->w;
    ...
    for (int i = dy; i < dy + h; i++)
        memset(dst->pixels + i * dst->w + dx, color, w);
}
