...
void syscallSemWait(struct StackFrame *sf) {
	int sem_index = sf->edx; // the index of the sem entry
	// P operation
	sem[sem_index].value--;
...
void syscallSemPost(struct StackFrame *sf) {
	int sem_index = sf->edx; // the index of the sem entry
	ProcessTable *pt = NULL; // use to release blocked processes
	// V operation
	sem[sem_index].value++;
	if (sem[sem_index].value <= 0)
	{
...
void syscallPid(struct StackFrame *sf)
{
	pcb[current].regs.eax = pcb[current].pid; // set return value
	return;
}