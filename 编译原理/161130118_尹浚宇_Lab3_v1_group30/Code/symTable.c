...
void initTable()//初始化符号表
{
	...
	//实验3所需,向符号表中加入read write函数
	SymList readFun = (SymList)malloc(sizeof(SymNode));
	strcpy(readFun->name,"read");//函数名
	readFun->type = (Type)malloc(sizeof(struct Type_));
	readFun->type->kind = FUNCTION;//函数类型
	
	Function function = (Function)malloc(sizeof(struct Function_));
	function->name = readFun->name;//函数名
	function->retType = (Type)malloc(sizeof(struct Type_));
	function->retType->kind = BASIC;//返回值
	function->retType->u.basic = INT_TYPE;
	function->paraType = NULL;//参数
	
	readFun->type->u.function = function;
	readFun->down = NULL;
	readFun->right = NULL;
	putSym(readFun);
	
	SymList writeFun = (SymList)malloc(sizeof(SymNode));
	strcpy(writeFun->name,"write");//函数名
	writeFun->type = (Type)malloc(sizeof(struct Type_));
	writeFun->type->kind = FUNCTION;//函数类型
	
	function = (Function)malloc(sizeof(struct Function_));
	function->name = writeFun->name;//函数名
	function->retType = (Type)malloc(sizeof(struct Type_));
	function->retType->kind = BASIC;//返回值
	function->retType->u.basic = INT_TYPE;
	//描述write函数的参数
	FieldList para = (FieldList)malloc(sizeof(struct FieldList_));
	para->name = (char*)malloc(64*sizeof(char));
	strcpy(para->name,"n");
	para->type = (Type)malloc(sizeof(struct Type_));
	para->type->kind = BASIC;
	para->type->u.basic = INT_TYPE;
	para->tail = NULL;
	function->paraType = para;
	
	writeFun->type->u.function = function;
	writeFun->down = NULL;
	writeFun->right = NULL;
	putSym(writeFun);
}
