...

{
	...
	int width = (sxy - 1) / 2; // 步长
	int x_center = x_curr + xoff; // 搜索的中心
	int y_center = y_curr + yoff; // 搜索的中心
	while (width >= 1)
	{
		// 搜索本轮的9个点
		for (int x_itr = -1; x_itr <= 1; x_itr++)
		{
			...
			{
				i = x_center + x_itr * width;
				j = y_center + y_itr * width;
		...
		// 本轮搜索完毕, 指定下一轮搜索的中心点
		x_center = MV_FRAME[0].x + x_curr;
		y_center = MV_FRAME[0].y + y_curr;
		// 搜索步长减半
		width /= 2;
	}
